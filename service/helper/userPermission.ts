...
const readGranted = await rl.question(
  `Grant DIR-READ access? "${JSON_DIR}" [y/n]\t`
);
const writeGranted = await rl.question(
  `Grant DIR-WRITE access? "${CSV_DIR}" [y/n]\t`
);
const netGranted = await rl.question(
  `Grant NETWORK access? "${localhost}:${httpPort}" [y/n]\t`
);

if (useHTTPS) {
  const httpsGranted = await rl.question(
    `Grant NETWORK https access? "${serviceIP}:${httpsPort}" [y/n]\t`
  );
  if (httpsGranted?.toLowerCase() !== "y") {
    throw new Error(`Denied NETWORK https access "${serviceIP}:${httpsPort}"`);
  }
}
...
if (netGranted?.toLowerCase() !== "y") {
  throw new Error(`Denied NETWORK access "${localhost}:${httpPort}"`);
}
if (!fs.existsSync(CSV_DIR)) {
  const createIt = await rl.question(
    `Create required directory? "${CSV_DIR}" [y/n]\t`
  );
...
if (!fs.existsSync(JSON_DIR)) {
  const createIt = await rl.question(
    `Create required directory? "${JSON_DIR}" [y/n]\t`
  );
  if (createIt?.toLowerCase() === "y") {
