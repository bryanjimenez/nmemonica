...
export function useConnectKanji() {
  ...
  const [r, ft, choiceN]:[boolean, typeof TermFilterBy[keyof typeof TermFilterBy],number] = useSelector(({ kanji }:RootState) => {
  ...
  const activeTags = useSelector(({ kanji }:RootState) => {
