 ...
 * @param {Error} error
 */
function /*static*/ getDerivedStateFromError(error) {
  const causeMsg =
 ...
 * @param {Error} error
 */
function componentDidCatch(dispatch, error) {
  // @ts-expect-error Error.cause
 ...
 * @param {function} setShakeIntensity
 */
function buildMotionListener(dispatch, motionThreshold, setShakeIntensity) {
  /**
...
/**
 * @typedef {(t: boolean) => boolean} F
 * @param {boolean} name
 * @param {(arg0: F) => void} toggleSection
 */
function collapseExpandToggler(name, toggleSection) {
  const icon = name ? (
...
export default function Settings() {
  ...
  const motionListener = useRef(
    /** @type {ReturnType<typeof buildMotionListener> | undefined} */ (
      undefined
    )
  );
  ...
  useEffect(() => {
    if (motionThreshold > 0 && motionListener.current === undefined) {
      ...
      getDeviceMotionEventPermission(
        ...
        (/** @type {Error} */ error) => componentDidCatch(dispatch, error)
      );
  ...
  const kanjiSelectedTags = Object.values(kanji).filter((k) =>
    k.tag.some((/** @type {string}*/ aTag) => kanjiActive.includes(aTag))
  );
