...
self.addEventListener("fetch", (e) => {
  ...
  if (url === dataVerURL) {
    ...
    e.respondWith(
      caches
        .open(appDataCache)
        .then((cache) => cache.match(fbURL + "/newest/words.json"))
      // .then((res) => {
      //   caches.open(appDataCache).then((cache) => cache.delete(fbURL + "/newest/words.json"));
