...
self.addEventListener("fetch", (e) => {
  ...
  } else if (url === "https://bryanjimenez.github.io/refresh") {
    console.log("[ServiceWorker] hard refresh");

    const a = ghURL;
    const b = ghURL + "/";

    caches.delete(appStaticCache).then(() => {
      console.log("deleted?");
      return caches.open(appStaticCache).then((cache) => {
        console.log("opened?");
        return cache.addAll([...cacheFiles, a, b]).then(()=>{console.log('added??')});
      });
    });
