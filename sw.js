...
const swVersionConst =  'cdd9cf861dc440011b5c33ff9e168466';
      ...
      const dbResults = dbOpenPromise.then((db) => {
        fetch(url)
          ...
          .then((blob) =>
            ...
            writeIDBItem(db, { url, blob }).then((dataO) => toResponse(dataO))
          );
      });
        ...
        return readIDBItem(db, url)
          ...
          .catch(() =>
            ...
            fetch(url)
              .then((res) => res.blob())
              .then((blob) =>
                writeIDBItem(db, { url, blob }).then((dataO) =>
                  toResponse(dataO)
                )
              )
          );
