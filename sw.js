...
self.addEventListener("fetch", (e) => {
  ...
  if (url === dataVerURL) {
    ...
    e.respondWith(
      caches
        .open(appDataCache)
        .then((cache) => cache.match(ghURL + "/new/words.json"))
      // .then((res) => {
      //   caches.open(appDataCache).then((cache) => cache.delete(url));
