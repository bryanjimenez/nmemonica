    ...
    if (!self.indexedDB) {
      ...
      const dbResults = dbOpenPromise.then((db) => {
        ...
        return fetch(newUrl)
          ...
          .then((blob) => {
            console.log(blob);
            return deleteIDBItem(db, query).then(() => {
              return writeIDBItem(db, { query, blob }).then((dataO) =>
                toResponse(dataO)
              );
            });
          });
