...
/**
 * @typedef {Object} swParams
 * @property {string} swVersion
 * @property {string} initCacheVer
 * @property {string[]} cacheFiles
 * @property {string} ghURL
 * @property {string} fbURL
 * @property {string} gCloudFnPronounce
 * @property {string} SERVICE_WORKER_LOGGER_MSG
 * @property {string} SERVICE_WORKER_NEW_TERMS_ADDED
 * @property {function} getParam
 * @property {function} removeParam
 * @property {string} authenticationHeader
 */

/**
 * @param {swParams} param0
 */
  ...
  swSelf.addEventListener("fetch", (e) => {
    ...
    if (url === dataVerURL) {
      ...
      if (!swSelf.indexedDB) {
        ...
        const dbResults = dbOpenPromise.then(
          (/** @type {IDBDatabase} */ db) => {
            countIDBItem(db);
    ...
    return dbOpenPromise.then((db) => {
      ...
      const getAllP = new Promise((resolve, reject) => {
        ...
        request.onsuccess = (event) => {
          ...
          const transactionDb = /** @type {IDBRequest<IDBDatabase>} */ (
            event.target
          ).result;
